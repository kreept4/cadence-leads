...
export default async function handler(req, res) {
  ...
  try {
    const response = await fetch("https://cc.freemodel.dev/v1/messages", {
      ...
      body: JSON.stringify({ model: "claude-3-haiku-20240307", max_tokens: 2500, messages: [{ role: "user", content: `Find ${leadCount} B2B leads for: "${query}". Return only a JSON array with: company, industry, location, contactName, contactTitle, email, website, linkedin, fitScore, fitReason, regulation, pitch, followUp.` }] }),
    });
