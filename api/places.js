...
export default async function handler(req, res) {
  ...
  try {
    // Step 1: Search for real businesses using Places API
    ...
    // Step 2: Format places into lead structure
    ...
    // Step 3: Use Claude to enrich with pitch and compliance info
    ...
    let leads;
    try { leads = JSON.parse(clean); } catch { return res.status(500).json({ error: "Enrichment parse failed" }); }
